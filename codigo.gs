...
function buscarCotizaciones(termino) {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    ...
    datosCotizaciones.slice(1).forEach(cotizacion => {
      const cliente = datosClientes.find(cliente => cliente[idClienteClientesIndex] === cotizacion[idClienteIndex]);

      if (cliente) {
        const idCotizacion = cotizacion[idCotizacionIndex];
        const nombreCliente = cliente[nombreClienteIndex];
        const empresaCliente = cliente[empresaClienteIndex];

        // Buscar por ID, nombre o empresa
        if (
          idCotizacion.includes(termino) ||
          nombreCliente.toLowerCase().includes(termino.toLowerCase()) ||
          empresaCliente.toLowerCase().includes(termino.toLowerCase())
        ) {
          resultados.push({
    ...
    Logger.log("Error en buscarCotizaciones: " + error.message);
    throw new Error(error.message);
  }
}
...
function obtenerTodasLasCotizaciones() {
  try {
    ...
    Logger.log("Datos de Cotizaciones: " + JSON.stringify(datosCotizaciones));
    Logger.log("Datos de Clientes: " + JSON.stringify(datosClientes));
    ...
    // Recorrer todas las cotizaciones (excluyendo la fila de encabezados)
    datosCotizaciones.slice(1).forEach(cotizacion => {
      const cliente = datosClientes.find(cliente => cliente[idClienteClientesIndex] == cotizacion[idClienteIndex]);

      Logger.log(`Buscando cliente para cotización ${cotizacion[idCotizacionIndex]}: ${cliente ? "Encontrado" : "No encontrado"}`);
    ...
    Logger.log("Resultados combinados: " + JSON.stringify(resultados));
    return resultados;
  } catch (error) {
